...
export default function DayChatPage() {
    ...
    createClient()
      ...
      .then(({ data }) => {
        if (data) {
          ...
          setInitialPrompt(urlPrompt ?? `Quero me preparar para a entrevista na ${data.company_name} — ${data.role_title}`)
        }
        setJobLoading(false)
      })
  ...
  const handleNewChat = useCallback(() => {
    ...
    router.push(`/${slug}/days/${dayNumber}/chat`)
  }, [router, slug, dayNumber])
    ...
    <div className="page-col">
      ...
      <div style={{ flex: 1, overflow: 'hidden', display: 'flex', position: 'relative' }}>
        <div className="chat-area" style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          {jobLoading ? (
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--tng-mute)', fontSize: 13, fontFamily: 'var(--tng-font-mono)' }}>

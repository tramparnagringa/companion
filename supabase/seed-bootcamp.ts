...
// ── AI instructions per day — used only for seeding the bootcamp program ──────
// These are NOT used at runtime. The live system reads ai_instructions from
// program_days in the DB. This map is the source of truth for the seed script.
...
## Rich text formatting for value propositions
When writing the value_proposition and value_proposition_alternatives, use inline markers so key phrases are visually highlighted in the dossier:
- **double asterisks** around the candidate's core skill or differentiating role (e.g. **builds financial infrastructure**) — renders in coral
- *single asterisks* around the impact phrase or key result (e.g. *enabling global fintech scale*) — renders with a lime highlight

Example: "Senior Backend Engineer who **builds financial infrastructure** that *enables fintech startups to scale globally*, without rebuilding their stack."
Use these markers in every value proposition you generate — they make the dossier more personal and readable.
